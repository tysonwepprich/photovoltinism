...
# Differences
# Springtime oviposition distribution unknown
# Spring development may start based on photoperiod or GDD
# No pupa lifestage

# Two photoperiod responses from N and S biotypes
  ...
  outfiles <- foreach(sim = 1:nsim,
    ...
    foreach(map = 1:length(SplitMap),
            ...
            .inorder = FALSE) %dopar%{
                ...
                for (i in stgorder) {  # Handle stages in the model
                  ...
                  if (i == "E" | i == "OE") {   # Egg Stage
                    ...
                    if (i == "OL") {
                      ...
                      Lifestage <- Cond(LSOW1 == 1 & progressOW1 == 1, 3, Lifestage)
                      #Reset the DDaccum cells to zero for cells that progressed to next lifestage
                      DDaccum <- Cond(progressOW1 == 1,(DDaccum - OWlarvaeDD) * LSOW1, DDaccum)
                    } else if (i == "L") {
                      progress1 <- (DDaccum * LS1) >= larvaeDD
                      Lifestage <- Cond(LS1 == 1 & progress1 == 1, 3, Lifestage)
              ...
              # memory management
              rm(DDaccum, LSOWstack, LS0stack, LS1stack, LS3stack, LS4stack, NumGenstack)
